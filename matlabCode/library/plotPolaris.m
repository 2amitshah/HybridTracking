...
if ~exist('Y', 'var')
    error('Please give the current Y to plotPolaris, otherwise the setting cannot be displayed correctly.')
%     Y = polaris_to_aurora([],H_OT_to_EMT);
end


%% Polaris base points and vertices
polarisBaseVertices = [
    75, -35, 35; % Upper part
...
%rotation
distancePolaris_to_Aurora = Y(1:3,4)';
...
%translation
polarisBaseVerticesTranslated = polarisBaseVerticesRotated + repmat(distancePolaris_to_Aurora,size(polarisBaseVertices,1),1);
...
centerCylinder1 = distancePolaris_to_Aurora + (rotEMT_to_OT*rotationZ90*rotationX90*[-45 -45 0]')';
centerCylinder2 = distancePolaris_to_Aurora + (rotEMT_to_OT*rotationZ90*rotationX90*[45 -45 0]')';
...
polarisTripodVertices = [-20 -20 35;
                        ...
                        20 20 35;
                        20 -20 35;
                        -20 -20 -distancePolaris_to_Aurora(3);
                        -20 20 -distancePolaris_to_Aurora(3);
                        20 20 -distancePolaris_to_Aurora(3);
                        20 -20 -distancePolaris_to_Aurora(3)] ;
polarisTripod1LVertices = [polarisTripodVertices(5:8,:);
                        -200 60 -2*distancePolaris_to_Aurora(3);
                        -200 100 -2*distancePolaris_to_Aurora(3);
                        -160 100 -2*distancePolaris_to_Aurora(3);
                        -160 60 -2*distancePolaris_to_Aurora(3)];
...
polarisTripod3LVertices = [polarisTripodVertices(5:8,:);
                        -20 -200 -2*distancePolaris_to_Aurora(3);
                        -20 -160 -2*distancePolaris_to_Aurora(3);
                        20 -160 -2*distancePolaris_to_Aurora(3);
                        20 -200 -2*distancePolaris_to_Aurora(3)];
                    
                    
polarisTripodVertices = polarisTripodVertices + repmat(distancePolaris_to_Aurora,size(polarisTripodVertices,1),1);
polarisTripod1LVertices = polarisTripod1LVertices + repmat(distancePolaris_to_Aurora,size(polarisTripod1LVertices,1),1);
polarisTripod2LVertices = polarisTripod2LVertices + repmat(distancePolaris_to_Aurora,size(polarisTripod2LVertices,1),1);
polarisTripod3LVertices = polarisTripod3LVertices + repmat(distancePolaris_to_Aurora,size(polarisTripod3LVertices,1),1);
